"use server";

import { revalidatePath } from "next/cache";
import { supabaseAdmin } from "@/lib/supabase/admin";
import { PAGE_SLUGS } from "@/lib/content/page-slugs";
import { aboutPageContent as aboutPageContentFallback } from "@/content/about";

type DeepWiden<T> =
  T extends string ? string
  : T extends number ? number
  : T extends boolean ? boolean
  : T extends readonly (infer U)[] ? DeepWiden<U>[]
  : T extends object ? { -readonly [K in keyof T]: DeepWiden<T[K]> }
  : T;

type AboutContent = DeepWiden<typeof aboutPageContentFallback>;

type SaveAboutResult =
  | { ok: true }
  | { ok: false; error: string };

export async function saveAboutContent(
  content: AboutContent
): Promise<SaveAboutResult> {
  try {
    const { error } = await supabaseAdmin
      .from("page_content")
      .upsert(
        {
          page_slug: PAGE_SLUGS.about,
          content_json: content,
          updated_at: new Date().toISOString(),
        },
        {
          onConflict: "page_slug",
        }
      );

    if (error) {
      console.error("Failed to save about content:", error.message);
      return { ok: false, error: error.message };
    }

    revalidatePath("/about");
    revalidatePath("/admin/pages/about");

    return { ok: true };
  } catch (error) {
    console.error("Unexpected error while saving about content:", error);
    return { ok: false, error: "Unexpected error while saving about content." };
  }
}