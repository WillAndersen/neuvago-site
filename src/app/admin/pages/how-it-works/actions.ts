"use server";

import { revalidatePath } from "next/cache";
import { supabaseAdmin } from "@/lib/supabase/admin";
import { PAGE_SLUGS } from "@/lib/content/page-slugs";
import { howItWorksPageContent as howItWorksPageContentFallback } from "@/content/how-it-works";

type DeepWiden<T> =
  T extends string ? string
  : T extends number ? number
  : T extends boolean ? boolean
  : T extends readonly (infer U)[] ? DeepWiden<U>[]
  : T extends object ? { -readonly [K in keyof T]: DeepWiden<T[K]> }
  : T;

type HowItWorksContent = DeepWiden<typeof howItWorksPageContentFallback>;

type SaveHowItWorksResult =
  | { ok: true }
  | { ok: false; error: string };

export async function saveHowItWorksContent(
  content: HowItWorksContent
): Promise<SaveHowItWorksResult> {
  try {
    const { error } = await supabaseAdmin
      .from("page_content")
      .upsert(
        {
          page_slug: PAGE_SLUGS.howItWorks,
          content_json: content,
          updated_at: new Date().toISOString(),
        },
        {
          onConflict: "page_slug",
        }
      );

    if (error) {
      console.error("Failed to save how-it-works content:", error.message);
      return { ok: false, error: error.message };
    }

    revalidatePath("/how-it-works");
    revalidatePath("/admin/pages/how-it-works");

    return { ok: true };
  } catch (error) {
    console.error("Unexpected error while saving how-it-works content:", error);
    return {
      ok: false,
      error: "Unexpected error while saving how-it-works content.",
    };
  }
}