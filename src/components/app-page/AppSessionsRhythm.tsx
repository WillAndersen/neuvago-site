...
type AppSessionsRhythmProps = {
  content: typeof appPageContent.sessionsRhythm;
};
        ...
        <div className="grid gap-12 lg:grid-cols-[0.92fr_1.08fr] lg:items-center lg:gap-16">
          <div className="max-w-xl">
            <p className="text-[0.72rem] font-medium uppercase tracking-[0.26em] text-[#b8aa99]">
              {sectionContent.eyebrow ?? "Session rhythm"}
            </p>
            ...
            <div className="mt-9">
              <CtaButton href={content.cta.href} label={content.cta.label} variant="light" />
            </div>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-1">
            {content.features.map((feature, index) => {
              ...
              return (
                <article
                  key={feature.title}
                  className="grid gap-5 overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.055] p-4 sm:p-5 lg:grid-cols-[0.48fr_1fr] lg:items-center"
                >
                  {image ? (
