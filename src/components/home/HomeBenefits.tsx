...
type HomeBenefitsProps = {
  content: typeof homepageContent.benefits;
};

export function HomeBenefits({ content }: HomeBenefitsProps) {
  return (
      ...
      <div className="mx-auto max-w-[92rem] px-5 py-16 sm:px-8 lg:px-12 lg:py-24">
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-[1.08fr_0.92fr] lg:items-stretch">
          <div className="overflow-hidden rounded-[2.25rem] border border-black/6 bg-[#eee5da] shadow-[0_28px_95px_rgba(31,31,28,0.09)]">
            <div className="relative min-h-[23rem] h-full">
              <Image
          ...
          <div className="divide-y divide-black/10 border-y border-black/10">
            {content.items.map((item, index) => (
              <article
                key={item.title}
                className="grid gap-3 py-5 sm:grid-cols-[3rem_1fr] sm:gap-5 sm:py-6"
              >
                <span className="text-xs font-medium uppercase tracking-[0.2em] text-[#927f68]">
