...
export function ProductLaunchBuyBox() {
  return (
    <section id="buy" className="scroll-mt-28 bg-[#f2eee8]">
      <div className="mx-auto grid max-w-[92rem] gap-8 px-5 py-16 sm:px-8 lg:grid-cols-[1.08fr_0.92fr] lg:items-start lg:gap-12 lg:px-12 lg:py-24">
        <div className="grid gap-5">
          <div className="overflow-hidden rounded-[2.4rem] border border-black/5 bg-[#fbf8f2] shadow-[0_30px_110px_rgba(31,31,28,0.1)]">
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            {galleryImages.slice(1).map((image) => (
              <article
                key={image.src}
                className="overflow-hidden rounded-[1.8rem] border border-black/5 bg-[#fbf8f2] shadow-[0_18px_70px_rgba(31,31,28,0.07)]"
              >
                <div className="relative aspect-[4/3]">
                  <Image
                    src={image.src}
                ...
                <div className="border-t border-black/5 px-5 py-4">
                  <p className="text-xs font-medium uppercase tracking-[0.22em] text-[#8a7f72]">
                    {image.label}
                  </p>
                </div>
              </article>
        ...
        <aside
          ...
          className="rounded-[2.4rem] border border-black/5 bg-[#fbf8f2]/88 p-6 shadow-[0_30px_110px_rgba(31,31,28,0.1)] backdrop-blur sm:p-8 lg:sticky lg:top-28"
        >
          <p className="text-[0.72rem] font-medium uppercase tracking-[0.28em] text-[#81766a]">
            Launch product
          </p>

          <div className="mt-5 flex flex-col gap-4 border-b border-black/8 pb-7 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <h2
          ...
          <div className="mt-7 grid gap-3 sm:grid-cols-2">
            {includedItems.map((item) => (
              <div
                key={item}
                className="rounded-[1.25rem] border border-black/6 bg-white/58 px-4 py-3 text-sm font-medium text-[#38342f]"
              >
                {item}
              </div>
            ))}
